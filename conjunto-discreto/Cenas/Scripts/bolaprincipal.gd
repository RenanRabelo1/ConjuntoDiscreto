...
func _on_mudou_poder(novo_poder: String) -> void:
	# Só toca o som se o poder for realmente DIFERENTE do atual (opcional)
	if poder_ativo != novo_poder:
		poder_ativo = novo_poder
		if ui:
			ui.destacar_botao(poder_ativo)
			ui.tocar_som_clique() # <--- TOCA O SOM AQUI
		...
		"subconjunto":
			...
			for e in meu_conjunto:
				if not outro.has(e):
					ok = false
					break
			if ui: ui.feedback_subconjunto(meu_conjunto, outro, ok)

func conjunto_para_texto(conjunto: Array) -> String:
	if conjunto.is_empty(): return "{ ∅ }"
	var s = []
	for e in conjunto: s.append(SIMBOLOS.get(e, e))
	return "{ " + ", ".join(s) + " }"
