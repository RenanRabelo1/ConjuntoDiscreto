...
# (O caminho exato para a sua tela de seleção de bolinhas/opções)
const CAMINHO_SELECAO_JOGADOR = "res://Cenas/options.tscn" 
...
func mostrar_pagina(index):
	...
	# Mudamos o texto para indicar que o próximo passo é escolher o visual
	btn_proximo.text = "Continuar" if index < PAGINAS.size() - 1 else "Escolher Cor"
...
func _on_btn_proximo_pressed():
	if pagina_atual < PAGINAS.size() - 1:
		...
		# Acabou a história, vai para a seleção de cor
		get_tree().change_scene_to_file(CAMINHO_SELECAO_JOGADOR)
...
func _on_btn_pular_pressed():
	# Pulou a história, vai direto para a seleção de cor
	get_tree().change_scene_to_file(CAMINHO_SELECAO_JOGADOR)


func _on_botao_pular_tudo_pressed() -> void:
	# Tratando também esse outro botão de pulo se ele for usado
	get_tree().change_scene_to_file(CAMINHO_SELECAO_JOGADOR)
